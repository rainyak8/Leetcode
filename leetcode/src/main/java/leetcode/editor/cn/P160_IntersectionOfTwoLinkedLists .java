...
//
// Related Topics 哈希表 链表 双指针 👍 2306 👎 0
...
public class Solution {
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
		...
		ListNode[] tempAs = new ListNode[30010];
		ListNode[] tempBs = new ListNode[30010];
		...
		for (int i = 1; i <= A; i++) {
			for (int j = 1; j <= B; j++) {
				//System.out.println(tempAs[i].val + " " + tempBs[j].val);
				if(tempAs[i] == tempBs[j]){
					intersectVal = tempAs[i];
					return intersectVal;
				}
			}
		}
